...
module "managed_redises" {
  source          = "git::https://github.com/AeternaModules/azurerm_managed_redis.git?ref=v4.80.0"
  managed_redises = local.managed_redises
}

module "managed_redis_access_policy_assignments" {
  source                                  = "git::https://github.com/AeternaModules/azurerm_managed_redis_access_policy_assignment.git?ref=v4.80.0"
  managed_redis_access_policy_assignments = local.managed_redis_access_policy_assignments
  depends_on                              = [module.managed_redises]
}

module "managed_redis_geo_replications" {
  source                         = "git::https://github.com/AeternaModules/azurerm_managed_redis_geo_replication.git?ref=v4.80.0"
